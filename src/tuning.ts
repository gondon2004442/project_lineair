...
export const TUNING = {
  ...
  /**
   * БЛАНК. Пустая форма, приказ об отмене: всё, что летит в радиусе,
   * аннулируется как неоформленное. Выход из безнадёжной ситуации ценой
   * ресурса — платишь бланком, а не здоровьем.
   */
  blank: {
    /** Запас в начале забега и потолок пополнения на входе на этаж. */
    refillTo: 2,
    /** Сколько бланков можно унести: выдачи копятся выше пополнения. */
    carryMax: 4,
    /** Радиус аннулирования, px. */
    cancelRadius: 300,
    /** Толчок, которым сотрудников отбрасывает от субъекта. */
    pushForce: 520,
    /** Небольшой урон по всем, кого достало. */
    damage: 1,
    /**
     * Замедление на применение. Считается шагами симуляции, поэтому в
     * реальном времени длится дольше ровно во столько, во сколько
     * замедлен ход: 0.18 с при доле 0.35 — это примерно полсекунды.
     */
    slowMoTime: 0.18,
    slowMoScale: 0.35,
    /** Расходящееся кольцо: сколько живёт и какой толщины. */
    ringTime: 0.35,
    ringWidth: 4,
    ringAlpha: 0.9,
    /** Искажение пространства по фронту кольца и ширина этого фронта. */
    warpPower: 0.014,
    warpWidth: 0.04,
  },
  ...
  weapon: {
    ...
    precise: {
      ...
      reloadTime: 0.95,
      /**
       * Запас патронов сверх обоймы, прирост из выдачи и вес формы при
       * розыгрыше выдачи. Запас конечный: кончился — форму придётся
       * менять, а не перезаряжать бесконечно.
       */
      reserveMax: 140,
      pickup: 40,
      pickupWeight: 5,
  ...
  floor: {
    ...
    spawnAttempts: 200,
    /**
     * Глубина участка, с которой становятся доступны планировки сложности
     * 2 и 3. Доля пути от начала этажа к приёмной.
     */
    difficultyMid: 0.34,
    difficultyDeep: 0.7,
  ...
  /**
   * ВЫДАЧА. Раньше предмет давали за каждый зачищенный участок — то есть
   * не давали выбора и не создавали дефицита. Теперь это бросок, и он
   * сам себя исправляет: за каждый участок без выдачи шанс растёт, при
   * выдаче сбрасывается к базе. Полоса невезения кончается сама, и это
   * главное, ради чего pity-таймер вообще существует.
   */
  reward: {
    /** Базовый шанс выдачи за зачищенный участок. */
    base: 0.22,
    /** Прирост за каждый участок без выдачи. */
    step: 0.09,
    /** Выше этого шанс не поднимается. */
    cap: 0.82,
    /** Доля бланков среди выдач. */
    blankShare: 0.3,
    /** Доля боезапаса среди оставшихся выдач. */
    ammoShare: 0.4,
    /** Доля допусков среди оставшихся выдач, остальное — приложения. */
    passShare: 0.25,

    /**
     * Делопроизводство: во сколько раз тяжелее кандидат, который
     * завершает распоряжение с уже подшитым. Падает за каждое собранное
     * распоряжение, ниже synergyMin не опускается.
     */
    synergyFactor: 1.8,
    synergyDecay: 0.25,
    synergyMin: 1,
  ...
  /**
   * ЛИЧНОЕ ДЕЛО: два скрытых стата, оба видны только под F3.
   *
   * ВЗЫСКАНИЕ растёт за нарушение процедуры — за порчу имущества сверх
   * нормы. Чем оно выше, тем чаще на участке оказывается сотрудник
   * «НА КОНТРОЛЕ»: живучее, бьёт вдвое, ходит быстрее. Это добровольная
   * ручка сложности: ломать мебель телекинезом удобно, но за удобство
   * контора спросит.
   *
   * ВЫСЛУГА растёт за участки, пройденные без единого попадания. Она
   * укорачивает кулдаун рывка и поднимает шанс выдачи.
   */
  record: {
    /** Сколько имущества можно испортить за забег без последствий. */
    breakAllowance: 3,
    /** Взыскание за каждую единицу сверх нормы и его потолок. */
    penaltyPerBreak: 1,
    penaltyMax: 12,
    /** Выслуга за участок без единого попадания и её потолок. */
    servicePerCleanRoom: 1,
    serviceMax: 10,

    /** Шанс «на контроле» за очко взыскания и его потолок. */
    controlChancePerPoint: 0.06,
    controlChanceCap: 0.45,
    /** Во сколько раз крепче, больнее и быстрее сотрудник «на контроле». */
    controlHp: 3.5,
    controlHpAdd: 10,
    controlDamage: 2,
    controlSpeed: 1.5,
    /** Прибавка к шансу выдачи за каждого «на контроле» на участке. */
    controlRewardBonus: 0.08,

    /** Выслуга: доля кулдауна рывка за очко и потолок скидки. */
    serviceCooldownStep: 0.05,
    serviceCooldownCap: 0.5,
    /** Вклад обоих статов в базовый шанс выдачи. */
    serviceRewardStep: 0.02,
    penaltyRewardStep: 0.015,
  },

  /**
   * ДОБЫЧА. Второй источник приложений помимо зачистки.
   *
   * Один дефицитный ресурс — ДОПУСК — работает на двух рынках сразу:
   * опечатанный шкаф отдаёт случайное, стол выдачи — названное по описи.
   * Что лучше, зависит от того, чего не хватает, и в этом весь смысл.
   * Шкаф вдобавок вскрывается бланком: тогда бланк тратится не на то,
   * чтобы выжить сейчас, а на то, чтобы получить предмет потом.
   */
  stash: {
    /** Сколько допусков на руках в начале забега и потолок. */
    passesStart: 1,
    passesMax: 4,
    /** Вероятность опечатанного шкафа на рядовом участке. */
    safeChance: 0.45,
    /** Смещение seed для добычи участка. */
    seedStride: 0x2545f491,
    /**
     * Радиус, с которого субъект дотягивается до шкафа или ячейки. Он
     * заведомо меньше шага между ячейками: иначе в зоне оказывались бы
     * сразу две, и оформлялась бы та, что ближе, а не та, что выбрали.
     */
    reach: 40,
    /** Габариты шкафа и ячейки выдачи. */
    safeRadius: 16,
    cellRadius: 12,
    /** Сколько ячеек на столе выдачи и шаг между ними. */
    deskCells: 3,
    deskGap: 70,
    /** Отступ добычи от точки входа на участок. */
    clearance: 110,
    /** Что достаётся из шкафа: сначала боезапас, потом бланк, иначе приложение. */
    safeAmmoShare: 0.3,
    safeBlankShare: 0.2,
  },
  ...
  prop: {
    /**
     * Укрытие: во сколько раз шире положенный набок предмет и сколько
     * попаданий держит. Габарит растёт ровно настолько, насколько растёт
     * то, что видно: хитбокс укрытия и есть его картинка.
     */
    coverWidth: 1.45,
    coverHits: 9,
  ...
  telekinesis: {
    ...
    /**
     * Искажение пространства. Захват и бросок — короткие волны, удержание
     * — постоянная мелкая рябь вокруг предмета: пока он висит, видно, что
     * держат его не руками.
     */
    grabWarp: 0.012,
    throwWarp: 0.018,
    holdWarp: 0.005,
    ...
    holdRadius: 0.05,
    /**
     * Короткое нажатие ПКМ кладёт предмет набок — это укрытие. Захват
     * по-прежнему мгновенный, отличается только отпускание: отпустил
     * сразу — поставил, подержал — бросил.
     */
    tapTime: 0.18,
    /** Левитация: на сколько предмет приподнят и как дышит. */
    liftHeight: 7,
    liftBob: 2.2,
    liftBobRate: 5.5,
    liftShadowScale: 0.55,
  },
  ...
  fx: {
     ...
     */
    /**
     * Общая аберрация по краям кадра. Стала считаться от центра экрана,
     * а не от центра текстуры фильтра, то есть на тех же числах бьёт
     * заметно сильнее — поэтому число уменьшено.
     */
    aberration: 0.06,
  ...
  render: {
    ...
    dashTrailStep: 0.035,
    /**
     * Двойной контур сотрудника «на контроле». Именно двойной и
     * неподвижный: одиночный мигающий контур уже занят телеграфом, и
     * спутать их в бою было бы дорого.
     */
    controlOutline: 2,
    controlOutlineGap: 4,
    /** Светлая кромка и обводка положенного набок укрытия. */
    coverLip: 4,
    coverEdge: 2,
    /** Обводка добычи, её пломба и отступ контура в зоне оформления. */
    stashEdge: 2,
    stashSeal: 4,
    stashReachPad: 5,

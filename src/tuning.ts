...
export const TUNING = {
  ...
  blank: {
     ...
     * замедлен ход: 0.13 с при доле 0.35 — это примерно 0.37 с.
     */
    slowMoTime: 0.13,
    slowMoScale: 0.35,
    /**
     * Расходящееся кольцо: сколько живёт и какой толщины. Вместе с
     * замедлением вся подача бланка занимает около 0.48 с реального
     * времени; при 0.35 и 0.18 она занимала 0.68 с и читалась как
     * затянутая.
     */
    ringTime: 0.24,
    ringWidth: 4,
  ...
  reward: {
    ...
    cap: 0.82,
    /**
     * Из чего состоит выдача за участок. Доли проверяются по очереди, и
     * что осталось — приложение к делу. Расходники здесь намеренно
     * редкие: бланки, допуски и патроны есть в шкафах и ячейках, а
     * зачистка участка — единственный постоянный источник приложений.
     * При долях 0.30/0.40/0.25 приложением становилась лишь треть
     * выдач, и распоряжение собиралось в 37 забегах из ста.
     */
    blankShare: 0.15,
     ...
     *
     * В ТЗ стояло «около 1.8». Столько не хватает: за забег выпадает
     * три приложения, то есть смещённых бросков всего два, а пар среди
     * десяти приложений девять из сорока пяти. При 1.8 треть забегов
     * набирала пару и не складывала её. Спад тоже поднят: при 0.25 от
     * пятёрки он ничего не значил, при 1 множитель гаснет к четвёртому
     * распоряжению — как и задумано.
     */
    synergyFactor: 5,
    synergyDecay: 1,
    synergyMin: 1,
  ...
  /**
   * ТАЛОНЫ. Служебная мелочь: сыплется с ликвидированного штата и
   * копится за забег. Это наши гильзы из Gungeon — валюта для стоек и
   * стола выдачи. Тратить их пока негде: стойки идут следующим слоем,
   * так что сейчас талон — только счётчик и повод подойти к телу.
   *
   * Не собранное остаётся на участке: уходя, талоны с пола не забирают.
   */
  ticket: {
    /** Сколько талонов оставляет рядовая ставка. */
    perStaff: 2,
    /** Надбавка за старшую ставку и за Заведующего. */
    seniorBonus: 3,
    chiefBonus: 10,
    /** «На контроле» платит больше: проверка — это и риск, и повод. */
    controlBonus: 2,
    /**
     * Разлёт талонов при выпадении и торможение бумаги о пол. Путь
     * листка — примерно spread/drag: при 110 и 7 он получался 15 px,
     * то есть кучка на месте тела вместо россыпи.
     */
    spread: 240,
    drag: 7,
    /** С какого расстояния талон тянется к субъекту и с каким ускорением. */
    magnetRadius: 120,
    magnetPull: 1100,
    /** Ближе этого талон считается собранным. */
    pickupRadius: 14,
    /**
     * Пауза перед притяжением. Без неё талон влетает обратно в субъекта
     * на первом же кадре и разлёта не видно вовсе.
     */
    pickupDelay: 0.25,
    /** Полсторона талона в кадре и его хитбокс. */
    size: 3,
    /** Разброс сущностей за один забег держим в узде. */
    maxOnFloor: 120,
  },

  /**
   * СТОЙКИ. Наши алтари: окошко, в которое подают один раз за забег.
   *
   * Цены считаны от того, что забег приносит: талонов за забег выходит
   * около восьмидесяти при худших пятидесяти семи, так что одна стойка
   * стоит примерно треть забега, а две — почти всё.
   */
  counter: {
    /** Сколько стоек на этаже. */
    perFloor: 2,
    /** Полсторона стойки и с какого расстояния её оформляют. */
    radius: 16,
    reach: 40,
    /** Регистрация: цена в талонах и сколько пунктов взыскания снимает. */
    registryPrice: 28,
    registryClear: 4,
    /** Кадры: сколько здоровья берут и сколько допусков дают. */
    hrHealth: 1,
    hrPasses: 1,
    /**
     * Переаттестация: цена, шанс, что повезёт, и на сколько двигает
     * выслугу или взыскание. Половина на половину — это и есть кубик.
     */
    reviewPrice: 22,
    reviewLuck: 0.5,
    reviewService: 3,
    reviewPenalty: 3,
    /** Свой поток случайности и отступ от входа при расстановке. */
    seedStride: 0x2f6a5b13,
    clearance: 110,
  },

  /**
   * СТОЛ ВЫДАЧИ. За ним стоит кладовщик, и платят ему талонами, а не
   * допуском: допуск открывает запертое, талоны тратят в выдаче. Раньше
   * ячейки брали допуск, и один дефицитный ресурс тянул два рынка,
   * которые документ развёл.
   */
  clerk: {
    /** Цена одной ячейки в талонах. */
    cellPrice: 34,
    /** Особая выдача: сколько благодарностей берёт и сколько даёт. */
    specialCommendations: 1,
    specialItems: 2,
    /** На сколько кладовщик стоит позади ряда ячеек. */
    standBack: 54,
    /** Насечек на его табличке и полсторона силуэта. */
    plateMarks: 4,
    radius: 11,
    /**
     * Выстрел в кладовщика стол закрывает, а в дело идёт взыскание:
     * порча имущества и есть нарушение процедуры.
     */
    offencePenalty: 4,
    noteTime: 2.5,
  },
  ...
  /**
   * АРХИВ. Смерть — это отзыв допуска, а не конец: старое дело уходит на
   * полку, и его можно найти следующим экземпляром. Смерти становятся
   * накопительными, а не стёртыми.
   */
  archive: {
    /** Сколько дел хранится на полке. */
    keep: 12,
    /** Вероятность, что на рядовом участке попадётся чужое дело. */
    chance: 0.3,
    /** Смещение seed для розыгрыша дела. */
    seedStride: 0x7f4a7c15,
  },
  ...
  post: {
    ...
    chief: {
      ...
      /**
       * Три фазы. Границы — доля оставшейся прочности: пока выше
       * phaseTwoAt, идёт первая; ниже phaseThreeAt — третья. С каждой
       * фазой циркуляр чаще и плотнее, а переназначения — злее.
       */
      phaseTwoAt: 0.66,
      phaseThreeAt: 0.33,
      /** Во сколько раз короче интервал циркуляра во второй и третьей фазе. */
      phaseTwoRate: 0.72,
      phaseThreeRate: 0.5,
      /** Сколько снарядов добавляется к кольцу в каждой следующей фазе. */
      phaseRingAdd: 4,
      /** Во сколько раз чаще переназначения в третьей фазе. */
      phaseThreeReshuffle: 0.55,
      /**
       * Смена фазы: заведующий замирает, и это единственное окно, когда
       * его бьют без ответа. Заодно он разом перетасовывает подчинённых.
       */
      phaseBreak: 1.1,
      phaseBreakShuffle: 3,

      /**
       * Благодарность в личное дело: приёмная, пройденная без единого
       * попадания, добавляет перманентный контейнер здоровья на забег.
       */
      commendationHp: 1,
    },
  ...
  render: {
    ...
    playerRim: 2,
    /**
     * Призраки, тянущиеся за рывком. Два, с шагом 8 мс: при скорости
     * рывка это 18 и 35 px позади — внутри пройденных 110 px. Прежние
     * три по 35 мс уводили хвост на 231 px, то есть туда, где субъект
     * не был: смаз врал о траектории.
     */
    dashTrail: 2,
    dashGhostAlpha: 0.55,
    ...
    dashTrailStep: 0.008,
    /**
     * Шаг субъекта: четыре ключевые позы на цикл, цикл — на walkStride
     * пути. Считаем по пройденному, а не по времени: иначе на
     * замедлении и на разгоне ноги «едут» отдельно от пола.
     *
     * Цикл в 34 px означал, что при скорости 290 px/с ключ держится
     * 1.76 кадра: тело меняло габарит почти каждый кадр, и походка
     * читалась как тряска. 88 px — это 4.6 кадра на ключ.
     */
    walkFrames: 4,
    walkStride: 88,
    /** Подъём тела на проходных кадрах и перенос веса на опорных, px. */
    walkBob: 2.4,
    walkLean: 1.6,
    /** Сплющивание на опорном кадре, доля стороны. */
    walkSquash: 0.1,
    /** Ниже этой скорости субъект считается стоящим. */
    walkMinSpeed: 20,
    /**
     * На какой прибавке к этой скорости походка набирает полную
     * амплитуду. Без разгона поза прыгала в полный размах на первом же
     * кадре движения и обратно на последнем.
     */
    walkEase: 150,
    /**
     * Разрыв, после которого счётчик шага не набегает: переход в другой
     * участок — это телепорт, а не пробежка.
     */
    walkJumpCut: 40,
    /** Дыхание на месте: два кадра, полный цикл 0.8 с. */
    idleBreathTime: 0.8,
    idleBreath: 0.05,
    /** Растяжение по оси движения на рывке. */
    dashStretch: 1.25,
    /**
    ...
    controlOutlineGap: 4,
    /**
     * Стойка: во сколько раз тумба шире своей глубины, высота столешницы
     * и на сколько табличка поднята над ней.
     */
    counterWide: 1.5,
    counterTop: 5,
    counterPlateLift: 7,
